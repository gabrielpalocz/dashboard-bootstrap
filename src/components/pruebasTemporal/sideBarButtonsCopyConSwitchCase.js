                                ...
                                case 'collapse':
                                    return (<>
                                        <div class="d-grid gap-2">
                                            <button class="btn btn-bd-primary border-0 rounded-3 pt-2 my-2 ms-1 " type="button" data-bs-toggle="collapse" data-bs-target="#collapseItems" aria-expanded="false" aria-controls="collapseItems" style={nonActiveStyle}>
                                                <div className='d-flex justify-content-start ms-1' key={`divl--${indexH}`}>
                                                    <item.icon key={`icon--${indexH}`} />&nbsp;<span key={`span--${indexH}`}>{item.title}</span>
                                                </div>
                                            </button>
                                        </div>
                                        <div class="collapse" id="collapseItems">
                                            <div class="card card-body border-0">
                                                {item.children?.map((colap, indexC) =>
